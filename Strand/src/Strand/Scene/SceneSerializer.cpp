...
namespace YAML {
	...
	template<>
	struct convert<Strand::UUID>
	{
		static Node encode(const Strand::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Strand::UUID& uuid)
		{
				...
				if (scriptComponent)
				{
					...
					if (scriptFields)
					{
						...
						for (auto scriptField : scriptFields)
						{
							...
							SD_CORE_ASSERT(fields.find(name) != fields.end());

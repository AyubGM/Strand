...
namespace Strand::Math {
	...
	struct Matitr {
		...
		Matitr(T* current, int increment, int length) : element(current), increment(increment), length(length)
		{
		}

		Matitr(const Matitr<std::remove_const_t<T>>& toconst) : element(toconst.element), increment(toconst.increment), length(toconst.length)
		{
		}

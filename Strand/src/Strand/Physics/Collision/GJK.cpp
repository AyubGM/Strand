...
namespace Strand 
{
	...
	template<>
	bool NextSimplex(
		Simplex<Dimension::D2>& points,
		_vec   <Dimension::D2>& direction)
	{
		switch (points.size()) {
	...
	_vec<Dimension::D2> tripleProduct(_vec<Dimension::D2> a, _vec<Dimension::D2> b, _vec<Dimension::D2> c) {
		_vec<Dimension::D3> A(a.x, a.y, 0);
		_vec<Dimension::D3> B(b.x, b.y, 0);
		_vec<Dimension::D3> C(c.x, c.y, 0);

		_vec<Dimension::D3> second = cross(cross(A, B), C);

		return _vec<Dimension::D2>(second.x, second.y);
	}
		...
		_vec   <Dimension::D2>& direction)
	{
		...
		if (SameDirection<Dimension::D2>(abf, ao)) { // should see if this is good enough (no ac check)
			return Line(points = { a, b }, direction);
		}

		if (SameDirection<Dimension::D2>(acf, ao)) {
	...
	template<>
	bool NextSimplex(
		Simplex<Dimension::D3>& points,
		_vec   <Dimension::D3>& direction)
	{
		switch (points.size()) {
		case 2: return Line(points, direction);
		case 3: return Triangle(points, direction);
		...
		_vec   <Dimension::D3>& direction)
	{
		...
		if (SameDirection<Dimension::D3>(ab, ao)) {
		...
		_vec   <Dimension::D3>& direction)
	{
		...
		if (SameDirection<Dimension::D3>(cross(abc, ac), ao)) {
			if (SameDirection<Dimension::D3>(ac, ao)) {
				points = { a, c };
			...
			if (SameDirection<Dimension::D3>(cross(ab, abc), ao)) {
				return Line(points = { a, b }, direction);
			}

			else {
				if (SameDirection<Dimension::D3>(abc, ao)) {
					direction = abc;
				}
		...
		_vec   <Dimension::D3>& direction)
	{
		...
		if (SameDirection<Dimension::D3>(abc, ao)) return Triangle(points = { a, b, c }, direction);
		if (SameDirection<Dimension::D3>(acd, ao)) return Triangle(points = { a, c, d }, direction);
		if (SameDirection<Dimension::D3>(adb, ao)) return Triangle(points = { a, d, b }, direction);
		...
		while (minDistance == FLT_MAX)
		{
			...
			if (abs(sDistance - minDistance) > 0.001f) {
				...
				for (size_t i = 0; i < normals.size(); i++) {
					if (SameDirection<Dimension::D3>(vec_t(normals[i]), support)) {
						size_t f = i * 3;

...
namespace Strand {
	...
	namespace Utils {
		...
		static GLenum StrandImageFormatToGLDataFormat(ImageFormat format)
		{
			switch (format)
			{
		...
		static GLenum StrandImageFormatToGLInternalFormat(ImageFormat format)
		{
			switch (format)
			{
		...
		: m_Specification(specification), m_Width(m_Specification.Width), m_Height(m_Specification.Height)
	{
		...
		m_InternalFormat = Utils::StrandImageFormatToGLInternalFormat(m_Specification.Format);
		m_DataFormat = Utils::StrandImageFormatToGLDataFormat(m_Specification.Format);
	...
	void OpenGLTexture2D::SetData(Buffer data)
	{
		...
		SD_CORE_ASSERT(data.Size == m_Width * m_Height * bpp, "Data must be entire texture!");
	...
	void OpenGLTexture3D::SetData(Buffer data)
	{
		SD_PROFILE_FUNCTION();
		uint32_t bpp = (m_DataFormat == GL_RGBA) ? 4 : 3;
		SD_CORE_ASSERT(data.Size == m_Width * m_Height * m_Depth * bpp, "Data must be for the entire 3D texture!");
		glTextureSubImage3D(m_RendererID, 0, 0, 0, 0, m_Width, m_Height, m_Depth, m_DataFormat, GL_UNSIGNED_BYTE, data.Data);
	}
	...
	void OpenGLTextureCube::SetData(Buffer data)
	{
		...
		uint32_t bpp = (m_DataFormat == GL_RGBA) ? 4 : 3;
		SD_CORE_ASSERT(data.Size == m_Width * m_Height * 6 * bpp, "SetData for cubemap requires data for all 6 faces!");
